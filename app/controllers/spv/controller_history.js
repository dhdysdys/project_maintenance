...
router.get("/", async function (req, res) {
    ...
    if(req.user.role == 1){
        ...
        var [get_req_id] = await model.get_request_by_id_user(req.user.id)
        var [get_id_req] = await model.get_history(get_req_id[0].approver_mekanik)

        var arr = [];
        if(get_id_req.length > 0){
            var [get_request] = await model.get_history_by_id(get_id_req[0].id_request)
            ...
            for(var i=0;i<get_request.length;i++){
                var dataCurr = {
                    ...
                    tanggalAprrove: moment(get_request[i].request_date).format('LL'),
                    tanggalDo: moment(get_request[i].status_request_mekanik_date).format('LL'),
                }
